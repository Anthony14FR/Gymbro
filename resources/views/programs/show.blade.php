    ...
    <div class="container mx-auto px-4 py-8">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <form action="{{ route('programs.destroy', $program) }}" method="POST" class="inline-block">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn btn-sm" onclick="return confirm('{{ __('Êtes-vous sûr de vouloir supprimer ce programme ?') }}')">{{ __('Supprimer') }} <i class="fa-solid fa-trash ml-2"></i></button>
                </form>
            </div>
        </div>
        ...
        <div class="card lg:card-side bg-base-300 shadow-xl mb-12">
            <figure class="lg:w-1/3">
                <img src="{{ asset($program->image) }}" alt="{{ __('Image du programme') }}" class="w-full h-64 lg:h-full object-cover" />
            </figure>
            <div class="card-body lg:w-2/3">
                <p class="text-lg"><i class="fa-solid fa-caret-right mr-2"></i> {{ $program->description }}</p>
                <p class="text-sm"><i class="fa-regular fa-calendar-check mr-2"></i> {{ __('Créé le') }}: {{ $program->created_at->format('d M Y') }}</p>
                <p class="text-sm"><i class="fa-regular fa-calendar-plus mr-2"></i> {{ __('Dernière mise à jour') }}: {{ $program->updated_at->format('d M Y') }}</p>
                <p class="text-sm"><i class="fa-solid fa-star mr-2"></i> {{ __('Nombre de jours') }}: {{ $program->exercises->groupBy('pivot.day')->count() }}</p>
            </div>
        </div>
        ...
        <div class="space-y-8">
            @foreach ($program->exercises->groupBy('pivot.day') as $day => $exercises)
                <div class="p-4 md:p-6 rounded-lg text-white">
                    <h2 class="text-2xl md:text-3xl font-bold mb-4"><i class="fa-regular fa-calendar mr-2"></i> {{ __('Jour') }} {{ $day }}</h2>
                    <div class="overflow-x-auto shadow-md p-3 md:p-5 bg-base-300">
                        <table class="table w-full text-center">
                            ...
                            <tr>
                                ...
                                <th class="px-1 md:px-2">{{ __('Illustration') }} <i class="fa-solid fa-image ml-1"></i></th>
                                <th class="px-1 md:px-2">{{ __('Exercice') }} <i class="fa-solid fa-dumbbell ml-1"></i></th>
                                <th class="px-1 md:px-2">{{ __('Répétitions') }} <i class="fa-solid fa-repeat ml-1"></i></th>
